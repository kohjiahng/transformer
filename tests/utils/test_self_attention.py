...
def test_ndim_exception(module):
    ...
    with pytest.raises(Exception) as ecp:
        out = module(inp)
    assert str(ecp.value) == "Wrong number of dimensions passed into SelfAttentionModule.forward: Expected 3, got 2"
def test_dim_exception(module):
    inp = torch.rand((3,10,4))
    with pytest.raises(Exception) as ecp:
        out = module(inp)
    assert str(ecp.value) == "Wrong embed dimension passed into SelfAttentionModule.forward: Expected 10, got 4"